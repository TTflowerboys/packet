...
# function by ttflowerboys@gmail.com
# url : threetowns.cn | flowerboys.cn
# date: 2017/02/12
 ...
 */
/**
 * https://github.com/qqzj/beautifulCodes/blob/master/authcode.php
 * [authcode RC4可逆加密算法]
 * @param  string  $string    [原始字符串]
 * @param  string  $operation [DECODE执行解密操作,ENCODE执行加密操作]
 * @param  string  $key       [加密/解密用的KEY]
 * @param  integer $expiry    [密文有效期,要求加密主机/解密主机时间同步]
 * @return string             [根据$operation,返回原文/密文,解密失败返回空字符串]
 */
...
function check_mobile($mobile){
    $mobilepreg = '/^(((13[0-9]{1})|(15[0-9]{1})|(17[0-9]{1})|(18[0-9]{1}))+\d{8})$/';
...
# http://www.thinkphp.cn/code/989.html
...
function SendMsm1($to, $content)
{
    if(isMobile($to)){
        $url = "http://www.282930.cn/SMSReceiver.aspx";
        $postdata=array('username'=>c('msmset.msmuser'),'password'=>c('msmset.msmpwd'),'targetdate'=>'','mobiles'=>$to,'xmlmode'=>'','Content'=>$content);
        $ch = curl_init();
        // 设置URL和相应的选项
...
function SendMsm($to, $content)
{
    $con=1;
    if(C('webconfig.ismsm')&&isMobile($to)){
        $flag = 0; 
        $params='';//要post的数据 
        $argv = array( 
            'name'=>'dxw245435',     //必填参数。用户账号
            'pwd'=>'62CE1D60A258F4B015004DEB5083',     //必填参数。（web平台：基本资料中的接口密码）
            'content'=>$content,   //必填参数。发送内容（1-500 个汉字）UTF-8编码
            'mobile'=>$to,   //必填参数。手机号码。多个以英文逗号隔开
...
// 截取文字
function showdesc($content,$length){
    $length = $length?$length:100;
    $content = strip_tags(html_entity_decode($content));
    //mb_substr(strip_tags(html_entity_decode($vo['content'])),0,10,'utf-8')
    $str = mb_substr($content,0,50,'utf-8');
    return $str;
}
/**
* 字符串转数组并倒序
* @param string $string      [传入字符串]
* @param string $symbol      [分割符，默认为','分割]
* @return array              [返回数组,数组倒序排列]
*/
function arrOrderByKey($string,$symbol=null){
...
* @return array              [返回数组,数组倒序排列]
*/
function countPrice($string,$symbol=null){
    $symbol = $symbol? $symbol: "-";
    $newArr = array();
    $array = explode($symbol, $string);
    $v = 0;
...
function getRank($rank){
    $rankrs=C('userRankArr');
    return $rankrs[$rank];
}
# 会员状态
function getUserStatus($v){
    $status=C('userStatusArr');
    return $status[$v];
}
# 会员点位
function userAreaArr($v){
    $area = C('userAreaArr');
    return $area[$v];
}
# 订单状态
