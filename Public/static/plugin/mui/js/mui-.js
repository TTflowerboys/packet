...
(function($, window, document, name) {
	...
	var handle = function(event, target) {
		if (target.tagName === 'A') {location.href=target.href;return true;}
...
(function($, window, document) {
	...
	var CLASS_ICON_USER = 'icon-username';
	var CLASS_ICON_PHONE = 'icon-mobile';
	var CLASS_ICON_PWD = 'icon-pwd';
	var CLASS_ICON_VERITY = 'icon-verity';
	...
	var CLASS_HIDDEN = 'mui-hidden';
	var CLASS_FOCUSIN = 'mui-focusin';
	var CLASS_SHOW = 'mui-show';
	var SELECTOR_ICON_CLOSE = '.' + CLASS_ICON_CLEAR;
	var SELECTOR_ICON_USER = '.' + CLASS_ICON_USER;
	var SELECTOR_ICON_PHONE = '.' + CLASS_ICON_PHONE;
	var SELECTOR_ICON_PWD = '.' + CLASS_ICON_PWD;
	var SELECTOR_ICON_VERITY = '.' + CLASS_ICON_VERITY;
	...
	var Input = function(element, options) {
		...
		if (~this.options.actions.indexOf('slider')) { //slider
			...
			if (~this.options.actions.indexOf('user')) {
				this.userActionClass = CLASS_ICON + ' ' + CLASS_ICON_USER + ' ' + CLASS_SHOW;
				this.userActionSelector = SELECTOR_ICON_USER;
			}
			if (~this.options.actions.indexOf('phone')) {
				this.phoneActionClass = CLASS_ICON + ' ' + CLASS_ICON_PHONE + ' ' + CLASS_SHOW;
				this.phoneActionSelector = SELECTOR_ICON_PHONE;
			}
			if (~this.options.actions.indexOf('pwd')) {
				this.pwdActionClass = CLASS_ICON + ' ' + CLASS_ICON_PWD + ' ' + CLASS_SHOW;
				this.pwdActionSelector = SELECTOR_ICON_PWD;
			}
			if (~this.options.actions.indexOf('verity')) {
				this.verityActionClass = CLASS_ICON + ' ' + CLASS_ICON_VERITY + ' ' + CLASS_SHOW;
				this.verityActionSelector = SELECTOR_ICON_VERITY;
			}
	...
	Input.prototype.initAction = function() {
		...
		if (row) {
			if (self.sliderActionClass) {
				...
				if (self.userActionClass) {
					self.userAction = self.createAction(row, self.userActionClass, self.userActionSelector);
				}
				if (self.phoneActionClass) {
					self.phoneAction = self.createAction(row, self.phoneActionClass, self.phoneActionSelector);
				}
				if (self.pwdActionClass) {
					self.pwdAction = self.createAction(row, self.pwdActionClass, self.pwdActionSelector);
				}
				if (self.verityActionClass) {
					self.verityAction = self.createAction(row, self.verityActionClass, self.verityActionSelector);
				}
...
/*	Input.prototype.userActionClick = function(event) {
		
	};
	Input.prototype.phoneActionClick = function(event) {
	...
	$.fn.input = function(options) {
		...
		this.each(function() {
			...
			if (this.type === 'range' && row.classList.contains('mui-input-range')) {
				...
				if (classList.contains('mui-input-user')) {
					actions.push('user');
				}
				if (classList.contains('mui-input-phone')) {
					actions.push('phone');
				}
				if (classList.contains('mui-input-pwd')) {
					actions.push('pwd');
				}
				if (classList.contains('mui-input-verity')) {
					actions.push('verity');
				}
